...
fun main() {
    fun part1(input: List<String>): Int {
        var increased = 0
        val inputNum = input.map { it.toInt() }
        inputNum.forEachIndexed  { index, num ->
            if (index >0 && num > inputNum[index-1]){
                increased++
            }
        }
        return   increased
    ...
    fun part2(input: List<String>): Int {
        var increased = 0
        val inputNum = input.map { it.toInt() }
        for (index in 1 .. inputNum.size-3){
//            val left = inputNum[index-1]+inputNum[index]+inputNum[index+1]
//            val right = inputNum[index]+inputNum[index+1]+inputNum[index+2]
            if (inputNum[index+2] > inputNum[index-1]){
                increased++
            }
        }
        return increased
